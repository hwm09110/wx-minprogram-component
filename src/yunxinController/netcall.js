// import NetcallWeixin from '../vendors/NIM_Web_Netcall_miniapp_v7.9.0.js'
// import NIM from '../vendors/NIM_Web_NIM_miniapp_v7.9.0.js'
// import Emitter from '../utils/emitter.js'

const NetcallWeixin = require('../vendors/NIM_Web_Netcall_miniapp_v7.9.0.js')
const NIM = require('../vendors/NIM_Web_NIM_miniapp_v7.9.0.js')
import Emitter from '../utils/emitter.js'

let app = getApp()
module.exports = class NetcallController {
  constructor(props) {
  ...
  bindNetcallEvent() {
    ...
    this.netcall.on('beCalling', (data) => {
      ...
      if (currentPage.route.includes('videocall') === false && app.globalData.isPushBeCallPage == false) { // 不在多人通话中，才提示
        console.log('beCalling currentPage', !currentPage)
        // if (!currentPage) {
          console.log('1111');
          wx.navigateTo({
            url: `/pages/index/videocall?beCalling=true&caller=${data.caller}&cid=${data.cid}&type=${data.type}`,
          })
          app.globalData.isPushBeCallPage = true
          return
        // }
        // let netcallGroupCallInfo = store.getState().netcallGroupCallInfo
        // console.log('netcallGroupCallInfo', netcallGroupCallInfo)
        // let pages = getCurrentPages()
        // let currentPage = pages[pages.length - 1]
